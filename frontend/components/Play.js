...
const Play = () => {
  ...
  const handleCreateRoom = async(e) => {
        ...
        if(response.data)
        {
            console.log('Time to redirect the page after joining or creating the room');
  ...
  const handleJoinRoom = async (e) => {
    ...
    try{
        const response = await axios(`http://localhost:8080/game/join-room?gameId=${gameId}` , {
          method : 'POST', 
          headers : {
              Authorization :  `Bearer ${localStorage.getItem('jwtToken')}`
            }
          ...
          <Card className="text-center">
            <Card.Body>
              ...
              <Form onSubmit={handleJoinRoom}>
                ...
                <Button variant="primary" className="me-2" onClick={handleCreateRoom}>
                  Create Room
                </Button>
